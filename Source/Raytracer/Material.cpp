...
{
   // <raycast hit point + raycast hit normal + random in unit sphere>
...
bool Dielectric::Scatter(const ray_t& ray, const raycastHit_t& raycastHit, glm::vec3& color, ray_t& scattered) const
{
    ...
    if (glm::dot(ray.direction, raycastHit.normal) > 0)
    {
        ...
        cosine = m_refractionIndex * glm::dot(ray.direction, raycastHit.normal) / ray.direction.length();
    }
    else
    {
    ...
    scattered = (random() < reflectProbability) ? ray_t{ raycastHit.point, reflected } : ray_t{ raycastHit.point, refracted };
    color = m_albedo;
